...
def connect_to_mongodb(
    ...
    connection_uri = f"mongodb://{cli_args.username}:{cli_args.password}@{cli_args.host}:{cli_args.port}"
    ...
    try:
        ...
        return client[cli_args.database]
...
def connect_to_mqtt(
    ...
    logger.info(f"Connecting to MQTT Broker: {cli_args.mqtt_host}:{cli_args.mqtt_port}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MQTT 2 MongoDB Logger Script")
    
    # Add arguments
    parser.add_argument("--host", help="MongoDB Host", required=True)
    parser.add_argument("--port", help="MongoDB Port", default=27017)
    parser.add_argument("--username", help="MongoDB Username", required=True)
    parser.add_argument("--password", help="MongoDB Password", required=True)
    parser.add_argument("--database", help="MongoDB Database to be Used", default="mqtt_logs")
    parser.add_argument("--mqtt-host", help="MQTT Host", default="localhost")
    parser.add_argument("--mqtt-port", help="MQTT Port", default=1883)
    parser.add_argument("--mqtt-username", help="MQTT Username", default="mosquitto")
    parser.add_argument("--mqtt-password", help="MQTT Password", default="mosquitto")
    parser.add_argument("--mqtt-topics", nargs='+', help="MQTT Topics (can )", required=True)
    
    # Parse arguments
    cli_args = parser.parse_args()
    
    # Get a logger object
    logger = get_logger(stdout=True, stdout_stream_handler=logging.StreamHandler())
